...
function flipsTheWords(str) {
  let words = str.split(" ");
  let invertedWord = [];

  for (let i = 0; i < words.length; i++) {
    let chars = words[i].split("");
    invertedWord[i] = chars.reverse().join("");
  }
...
function capitalizeFirstLettersWithoutSpaces(str) {
  let words = str.split(" ");
...
function overwriteStringsInUppercase(object) {
