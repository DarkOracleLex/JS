...
// and the new sum is re-invested yearly after paying tax 'T' on the interests
// that were just gained (and only on the interests part).
...
function calculateYears(principal, interest, tax, desired) {
  let i = 0;
  for (i; principal < desired; i++) {
    let yearInterest = principal * interest;
    let yearTax =
      yearInterest - (yearInterest - yearInterest * tax);
    principal += yearInterest - yearTax;
  }
  return i;
}
